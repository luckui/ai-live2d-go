...
function buildChildSystemPrompt(task: DBTask): string {
  ...
  if (task.type === 'cron') {
    // 定时任务：可能是提醒、也可能是执行代码/检查数据等，speak 是可选的最后一环
    parts.push(
      '- 这是一个定时触发的任务，根据任务内容决定是否需要告知用户',
      '- 如果任务目的是提醒用户、播报结果或传达信息，在完成后调用 speak 工具通知用户',
      '- speak 是你与用户沟通的唯一渠道；如果不调用 speak，用户不会感知到任何输出',
      '- 如果任务是纯后台操作（如定时写文件、定时清理），无需 speak，直接输出结果摘要即可',
      '- 完成后用简洁的自然语言输出结果摘要',
    );
  } else {
    parts.push(
